...
function TestSeries() {
  ...
  const HandleEdTest = ()=>{
    ...
    setEdTests((prev)=>([...prev,{ id:Date.now(),testTitle:TestTitle,duration:testDuration ,problems:problems,questionType:problemTypes,New:false}]))
    setTestfrom(false)
    setDuration("");
    setEdTests("");
    setProblem("");
    ...
    <div className='py-10 lg:space-y-2'>
      ...
      <div className="Add a test">
        <div className=" space-y-4">
          <span className=' text-3xl font-semibold text-slate-500  '>New Test:</span>
          <div className=" h-60 border flex items-center justify-center">
            <button onClick={()=>{setTestfrom(true)}} className='text-[5rem] border h-32 w-32 flex items-center justify-center rounded-full bg-gray-400 text-white '>+</button>
          </div>

        </div>
        <div className={` ${edTestForm?"":"hidden"}   flex items-center justify-center absolute z-[4]  w-full left-0 h-screen top-0 bg-[#403f3fcf] `}>
       ...
       <div className="flex items-center justify-center border-b-4  ">
           <span className=' text-2xl font-bold text-gray-500  '> New Test</span>
           {/* <div className="">X</div> */}
          </div>
          <div className="px-4 space-y-4">
            <div className="testTitle space-x-2 items-center flex justify-between">
              <label htmlFor=" " className=' text-[23px] font-semibold '>Test Title:</label>
              <select name=""  onChange={(e)=>{
                setTestTitle(e.target.value)
               


              }} value={TestTitle} className=' w-[50%] p-1 outline-none focus:outline-none ' id=" SelectTitle">
            ...
            <div className="flex justify-between items-center">
              ...
              <input onChange={(e)=>{setDuration(e.target.value)}} value={testDuration} type="number" className=' w-[50%] outline-none p-1' placeholder='...Time' name="" id="" />
            </div>
            <div className="flex justify-between  items-center">
              <label htmlFor="">No of Questions</label>
              <input onChange={(e)=>{setProblem(e.target.value)}} value={problems} type="number" className=' w-[50%] outline-none p-1' placeholder='...type here' name="" id="" />
            </div>
            <div className=" flex justify-between items-center">
          ...
          <div className=" flex items-center space-x-2 justify-center">
            <button onClick={HandleEdTest} className=" border px-2 hover:bg-gray-900  bg-gray-700 transition-all duration-300 text-white rounded-lg p-1">Create New Test</button>
            <button onClick={()=>{ setTestfrom(false)}} className='  border px-2 bg-red-500 hover:bg-red-600 transition-all duration-300  text-white rounded-lg p-1'>Cancel</button>
          </div>
       </div>
      ...
      <div className="Your All tests ">
        <div className=" lg:space-y-4">
          <span className=' text-3xl font-semibold text-slate-500'>Your All Test:</span>
